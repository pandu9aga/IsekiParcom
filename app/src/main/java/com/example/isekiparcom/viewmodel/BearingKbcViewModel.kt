...
class BearingKbcViewModel(private val context: Context) : ViewModel() {
    ...
    val partDetectionResult = mutableStateOf<String?>(null) // "shaft bearing" atau "metal" atau apapun
    val showSecondCaptureButton = mutableStateOf(false) // Tombol untuk ambil foto OCR (muncul jika part = shaft bearing)
    val showRetakePartButton = mutableStateOf(false) // 🔥 Tombol ambil ulang foto part (muncul jika part != shaft bearing)
    ...
    val textRecordForUpload = mutableStateOf<String?>(null) // Selalu "KBC"
    ...
    fun processPartImage(bitmap: Bitmap) {
        val predictedClass = tflitePart.run(bitmap) // Harus mengembalikan "shaft bearing" atau "metal" atau apapun
        ...
        } else { // Jika bukan shaft bearing (metal, dll)
