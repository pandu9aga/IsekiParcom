...
class BearingShaftViewModel(private val context: Context) : ViewModel() {
    ...
    val capturedBearingPhotoFile = mutableStateOf<File?>(null)
    val bearingPhotoWithBoxes = mutableStateOf<Bitmap?>(null)
    val ballCount = mutableStateOf<Int?>(null)
    ...
    val lastLiveBitmap = mutableStateOf<Bitmap?>(null)
    val lastLiveDetections = mutableStateOf<List<Detection>>(emptyList())

    val isCaptureLocked = mutableStateOf(false)
